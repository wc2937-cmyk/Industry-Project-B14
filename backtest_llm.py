...
class BacktestLLMAnalyst:
    """
      LLM helper for generating a natural-language review of backtest results.

      It is backend-agnostic: as long as the endpoint follows the
      OpenAI-style /chat/completions API, it can work with Groq, DeepSeek,
      OpenAI, or other providers.

      Parameters
      ----------
      api_key : Optional[str]
          API key for the LLM provider. If None, requests will be sent
          without Authorization header.
      base_url : str
          Chat-completions endpoint URL.
      model : str
          Model name to use at the provider.
      verbose : bool
          If True, print raw prompts for debugging.
      """
    ...
    # ----------------- Helper: build a compact summary of performance table-----------------
    def _build_summary_text(
        ...
        """
        Build a short summary of equity curves (final, max, min).
        equities: dict keyed by (Ticker, Model) -> pd.Series of equity over time.
        """
        df = perf_df.copy()

        # Limit the size to avoid prompts becoming too long.
        if len(df) > 80:
            df = df.groupby(level=[0, 1]).head(1)

        # Sort them for easier viewing by both people and LLM.
    ...
    # -----------------Main function: Generative analysis-----------------
    def analyze(
        self,
        perf_df: pd.DataFrame,
        ...
        """
        `perf_df`: The performance table you construct in `main()` (MultiIndex: [Ticker, Model])
        `equities`: {(ticker, model_name) -> equity_curve_series}, optional
        `params`: Backtesting parameters (hold, fee, state_window, etc.)
        `language`: "zh" outputs Chinese, "en" outputs English
        """
        if params is None:
            params = {}
        ...
        # Optional: Add some net asset value (NAV) information (do not paste the entire curve to avoid it being too long).
        equity_summary = ""
